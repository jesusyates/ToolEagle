...
### 六点三、Git 上传指令（全自动完整上传 · 🔒）

**触发**：用户消息为 **上传 / 推到 GitHub / push / 提交并推送** 等明确「整仓上传」意图时（含「上传指令」表述），助手须 **自动执行到底**，**不要**只提交部分路径后停手。

**范围（完整）**：在本地校验通过后，`git add` **当前工作区全部有意变更**（含 **`content/blog` 等文章**、**SEO / 程序化内容**、**`generated/*` 中与本次工作相关的产物**、**`docs/MEMORY.md`**、**源码与配置** 等），**不**为「省 diff」而擅自排除文章或 SEO；除非用户在同一条消息里 **明确列出排除项**。

**流程（必须）**：

1. `git status` 看清变更面；必要时 `git diff --stat`。
2. **本地健康检查**：至少 `npx tsc --noEmit`；若本次改动含测试相关逻辑，再跑对应 `npm test`（或项目约定脚本）。**失败则不上传**，先修或向用户说明阻塞点。
3. `git add -A`（或等价：加入全部待提交变更）；若有 **不应提交** 的本地私货（如 `.env`、仅本机缓存），依 **`.gitignore`** 与仓库惯例排除，**不得**把密钥写进仓库。
4. `git commit -m "…"` 用 **一句完整说明** 概括本次批量变更主题。
5. `git push origin`（当前分支，一般为 `main`）。

**例外**：推送需 **网络/凭证** 且环境无法代劳时，用中文写 **「需您手动操作」**：在本地完成 `git push` 或配置 SSH/token。
