...
**共识**：代码量增长时，**结构须保持清晰、可维护**；线上须 **流畅、稳定**；出问题时 **易缩小范围、易排查根因**。（与「零」第 2 条「可维护、可观测、可演进」一致，本条为**落地约束**。）**多端模块化、shared-core 真相源、标准模块名与认证唯一链路**见下条 **「零点五·二」**；**Web / App / Mobile 共用一套全球规则层**、四核心字段、六层系统、目标数据模型与 SEO 主链见 **「零点五·三」**。
...
---
## 零点五·三、三端全球统一规则层（Web / App / Mobile · 🔒 永久）
**定位**：网站、**未来** App、**未来** 移动端 **共用一套全球规则层** — **不是**「网站一套、应用一套、移动端一套」三套并行口径。须 **统一市场规则、统一语言规则、统一能力定义、统一内容源、统一配置中心**。**执行顺序**：可以 **先** 网站英文跑稳；**架构上** 自始即按 **全球化 / 三端可复用** 留位，避免后续改名与双轨。  
**与「零点五·二」**：后端真相与模块边界仍服从 **shared-core**；本条补全 **全站字段语义、分层模型、数据与 SEO 管线口径**，供页面、接口、配置与内容生产 **一致引用**。API 若已使用 `contentLanguage`，**语义上等同于**本条 **`language`（内容语言）**，新代码优先与四字段命名 **对齐或显式映射**，**禁止** 同义再发明新字段名（如 `locale_code` vs `locale` 混用且无文档）。

### 一、总原则（三端一口径）
- **统一规则层**：市场、语言、能力、内容源、配置 **只有一套「全球规则」**；Web / App / Mobile **只体现不同职责与入口**，**不**各自定义一套枚举或命名。
- **职责可分流、规则不分流**：见下 **「五、网站 / 应用 / 移动端各自怎么接」**。

### 二、必须统一的 4 个核心字段（全局基础 · 禁止三端乱命名）
以后 **页面、接口、内容、功能配置** 凡涉及全球化语境，**须能落到**（或显式映射到）下列四字段；**禁止** Web 用一套、App 用另一套别名而不做映射。

1. **`market`** — 国家/市场（示例：`US`、`CN`、`JP`、`DE`、`FR`；亦含 **`global`** 等聚合口径时须在配置层定义，**禁止**口头混用 `CN` 与 `zh` 指代市场）。
2. **`locale`** — 地区/区域格式（BCP 47 类，示例：`en-US`、`zh-CN`、`ja-JP`、`de-DE`、`fr-FR`）。
3. **`language`** — 内容语言（示例：`en`、`zh`、`ja`、`de`、`fr`）。**与代码字段**：已有 **`contentLanguage`** 与本项 **同义**，新功能以本条为准对齐命名或文档。
4. **`variant`** — 品牌/站点/渠道变体（示例：`global`、`creator`、`business`、`mobile`、`app`）。用于 **同一 market+locale+language 下** 的展示/实验/渠道差异，**不**替代 `market`/`locale`。

### 三、三端统一的 6 层系统（概念层 · 实现逐步收敛）
下列分层 **口径统一**；物理存储可以是 DB、配置仓库、文件管线，但 **语义与归属** 须清晰，**禁止** 半套在库、半套在文件且无 canonical 说明。

1. **配置层** — 国家/语言、默认 `locale`、默认货币、功能开关、默认文案、SEO 策略等。  
2. **内容层** — SEO 文章、landing 文案、功能说明、onboarding、帮助中心等 **可版本化内容**。  
3. **能力层** — **统一能力名与能力键**（示例类：`AI writing`、`AI chat`、`AI automation`、`prompt assistance`、`personalized AI`）；**禁止** 网站一名、应用另一名而无 `feature_key` 映射。  
4. **SEO 层** — 关键词池、场景映射、文章生成、发布规则、`canonical` / `sitemap` / `hreflang` 等 **统一规则**。  
5. **运营层** — CTA、导流路径、推荐模块、内链策略、转化页等 **统一策略口径**。  
6. **合规层** — 各市场隐私、`Terms`、数据处理说明、年龄限制、支付合规等 **统一建档与分区展示**。

### 四、统一数据模型（目标形态 · 与实现演进对齐）
无论 Web / App / Mobile，**尽量**围绕下列 **核心表或等价结构**（名称可随迁移调整，**语义**须一致）：

1. **`markets`** — 市场配置：建议字段含 `code`、`name`、`default_locale`、`default_language`、`currency`、`timezone`、`is_active` 等。  
2. **`locales`** — `locale` 配置：建议 `code`、`language`、`region`、`market`、`is_default` 等。  
3. **`feature_registry`** — 统一功能定义：建议 `feature_key`、`feature_name`、`feature_group`、`description`、`supported_markets`、`supported_languages`、`supported_platforms` 等。  
4. **`content_items` / `seo_articles`**（或等价）— 统一内容源：建议含 `id`、`title`、`slug`、`description`、`content`、`status`、`deleted`、`market`、`locale`、`language`、`content_type`、`feature_key`、`created_at`、`updated_at` 等。  
5. **`seo_seeds`**（或与现有 `app-seo-seeds` / 管线输入对齐）— 统一 SEO 输入源：如 `feature`、`platform`、`keywords[]`、`steps[]`、`angles[]`、`markets[]`、`languages[]`、`notes`、`selling_points` 等。

### 五、网站、应用、移动端各自怎么接（职责不同 · 底层规则相同）
- **网站 Web**：SEO 流量入口、guides、tools、landing pages、内容后台（如 **`/admin/seo`**）等。  
- **应用 App**：深功能承接、自动化执行、高转化与留存场景。  
- **移动端 Mobile**：轻量使用、高频场景、消息/提醒/快捷入口、低门槛转化。  
**铁律**：三端 **职责**可分，**market / locale / language / variant** 与 **六层系统** **必须** 共用底层规则，**禁止** 各端私造「第二套真相」。

### 六、URL 与站点结构原则（须全局统一选型）
未来支持全球本土化时，**目录策略须二选一并在全仓统一**，**禁止**长期混用两套语义（例如同一产品同时混「纯 locale 路径」与「纯 market 路径」且无映射）。

- **示例 A（locale 目录优先，推荐方向）**：全局默认入口、`/en/`、`/zh-cn/`、`/ja-jp/`、`/de-de/` 等（具体段名以最终选型为准）。  
- **示例 B（market 目录）**：`/us/`、`/cn/`、`/jp/` 等。  

**与当前仓库**：**现状**以既有路由为准（如 **`/zh`** 中国站）；**演进**时须制定 **迁移/兼容/hreflang** 方案后再切换，**禁止** 无 Spec 的大改 URL 体系。倾向：**locale 目录优先** 对 **SEO 意图** 更清晰（本条 **建议方向**）。

### 七、SEO 系统统一规则（三端共用的内容生产主链）
现有 SEO 管线 **即** 三端统一内容生产入口的 **主链**；流程 **固定为**：  
**`seeds` → scenario mapper → preflight → draft generation → publish readiness → publish**。  
以后：**网站文章**、**应用 landing**、**移动端帮助内容** 等 **凡走程序化/规模化内容**，**优先** 复用本链或子步骤，**禁止** 另起一套无检索、无预检的「影子 SEO 工厂」而不标注。

### 八、图片与媒体规则（须提前纳入全球化）
**不能**只管理文字。内容模型 **建议** 统一支持：`cover_image`、`inline_images[]`、`image_alt_text[]`、**locale/market 维度的媒体变体**（不同市场 **图例/UI 截图/功能示例** 可不同）。图片与媒体 **须可按** `market` / `locale` **分区**，与文案策略一致。

### 九、支付与转化层（按市场预留）
全球化 **不仅是语言**：每一市场 **至少** 预留 **支付方式、定价展示、货币、税务提示、付款入口** 等产品与合规位；基础设施限制（如部署平台收款能力）**须在架构上记录**并 **不** 用「临时绕开」替代长期模型（与 **「二」分区支付**、**一点六** 副站边界 **叠加**）。

### 十、现在立刻执行的版本（当前阶段约束）
- **保留**：**数据库** 作为 **主要/唯一内容真相源**（与既有实现一致）；**`/admin/seo`** 为 **统一内容运营后台**；**英文先跑**；管理端 **`market` / `locale` / `language`（或 `contentLanguage`）** 控件 **已下拉化**的须 **延续**，新入口 **禁止** 退回不可控自由文本默认值。  
- **新增门禁（新功能上线须同步）**：  
  1. **`feature_registry` 记录**（或等价注册表，直至表落地）；  
  2. **`seo_seeds`（或等价种子源）** 可追溯记录；  
  3. **明确的 `market` / `locale` / `language`（及需要的 `variant`）支持范围**（文档或配置中可审计）。  
  **未完成上述补全的「全球化相关新功能」视为未闭环，默认不视为可发布态**（负责人特批除外，须在 MEMORY 或状态报告中留痕）。

### 十一、执行优先级
- **P0（现在就按此做）**：英文站稳定生产；功能与主题 **进 seeds**；统一内容源 **继续以数据库为主**；**三端统一字段命名**（Web 侧先落地）。  
- **P1（下一阶段）**：中英双语验证；locale 级内容结构；`hreflang` / 多语言 `sitemap`；`feature_registry` **正式化**（表+流程）。  
- **P2（后续）**：App/Mobile **共用同一配置中心**；多国家支付完善；多语言图片/媒体；多市场 A/B 文案等。

### 十二、最终铁规则（禁止项）
**禁止**：网站与应用 **同一能力不同名**；网站写 **`zh`**、应用只写 **`CN`** 指代混乱；SEO 用 **`language`**、移动端偷偷改用 **`locale_code`** 且无映射；**内容一半数据库一半文件** 且无 canonical；**同一市场三端文案口径不一致** 且无 `variant`/实验说明。  
**一句话**：未来是 **Web + App + Mobile 三端全球本土化**，**底层只能有一套统一规则系统**；**现在先把英文跑稳** 是 **执行顺序**，**架构上** 必须 **已为全球化留好位**。
