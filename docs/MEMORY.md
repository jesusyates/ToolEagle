...
### 六点三、上传指令（全自动：版本就绪 + EN/ZH 日更 SEO/AI 搜索发布准备 + GitHub · 🔒）

**Tool Eagle Upload + Daily SEO Publish — 永久执行约定**（用户简称 **「上传」** 即触发整条链路，**不等用户再补充一句才继续**）。

#### 触发词（等价）

**上传**、`upload`、**提交到 GitHub**、**push**、**提交代码**、推到 GitHub、提交并推送 等 —— 一律视为要求助手 **自动跑完** 下列流程（能执行的步骤不中断等待用户填话）。
...
- **生成物与合约**：若本次周期包含 Agent/SEO 管线，须 **同步** 所需的 `generated/*`、合约 JSON、脚本产出物（与 `scripts/`、`src/lib/agent/workflows.contract.json` 等保持一致），避免「代码已改、artifact 未更新」。

#### B. SEO / AI 搜索内容与日更发布准备（非「仅代码」）

- **默认心态**：日更发布是 **常驻生产职责**，不是可选项。用户发「上传」时，助手应假定：**当日代码改动**、**当日 EN/ZH 发布进度**、**当日 SEO/AI 搜索向内容产出** 在适用时应 **一并打包进本次提交**（见下条 C 的范围）。
- **范围（全站）**：**英文主站 + 中文站（`/zh`）** 每日都要推进；内容包括但不限于：SEO 文章、AI 搜索/答案引擎友好文章、程序化 SEO、以及发布系统依赖的队列/摘要/artifact（与 **「四、国家站模板」** 中 **每日 SEO 发布节奏**、质量门禁、Sitemap 规则 **互见**，不在这里重复全文）。
- **体量规则**：**不设**「每天固定 10 篇/20 篇」这类默认死数字；在 **不触发搜索引擎异常信号** 的前提下，按 **当前安全阈值下的最大安全量** 执行，由 **自动脚本/策略（动态推量、错峰、throttle）** 决定当日体量，而非人为保守少发。
- **安全**：遵守反异常、去重、多样性（topic / 页面类型 / 平台 / locale）、scale policy、降档/暂停规则；仅在风控要求时 **必要地** 减量，不过度收缩。
- **AI 搜索**：发布内容须兼顾 **传统搜索** 与 **AI 答案/摘要/引用** —— 可引用结构、摘要、可检索主题覆盖、高意图扩展、易被摘录引用的格式等（与站内已有 citation/capture 等能力 **对齐实现**，不凭空承诺平台效果）。
- **一侧阻塞**：若 EN 或 ZH 一侧因技术问题无法推进，**另一侧继续**；在短报告里 **标明阻塞侧与原因**、**最小修复建议**，恢复后尽快双侧并行。

**具体生成/发布命令** 以仓库 **`package.json` scripts** 与既有管线为准（如 `npm run audit:text`、英文 blog/程序化、中文程序化、`asset-seo:*` 等）；助手应 **主动查找并执行** 与「当日上传周期」匹配的脚本，而非等用户口述命令。

#### C. GitHub 上传准备（提交范围）

- **干净边界**：提交范围 = **本次版本相关改动** + **本次日更/发布周期必须纳入的内容与 artifact**；**排除** 与本次无关的本地漂移（误改、私货、纯本机缓存）。**不得** 提交 `.env`、密钥；遵守 **`.gitignore`**。
- **不要** 为省 diff 故意丢掉 **本应随本次发布上线的** 文章、`generated` 产物或合约更新。
- **流程**：`git add`（按上述范围）→ `git commit -m "…"`（一句概括）→ `git push origin`（当前分支，一般为 `main`）。

#### D. 上传完成后的反馈（短、可扫读）

助手须简要给出（不必长文）：

- 代码校验结果（tsc / test / build 要点）
- **EN** 发布准备/纳入情况
- **ZH** 发布准备/纳入情况
- 当日发布 **计划/已用体量**（动态上限语境下说明，非死数字）
- **安全/风控** 状态（是否正常、是否降档）
- GitHub **是否已推送**（分支与 commit 提示）

#### 例外

- 推送需 **网络/凭证** 且环境无法代劳时，用中文写 **「需您手动操作」**：本地 `git push` 或配置 SSH/token。
- 缺密钥、须本人账号的操作，同上 **「需您手动操作」**，并说明已完成的本地步骤。
