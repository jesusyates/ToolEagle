...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const rl = checkAuthRateLimit("signup_verify", ip, email);
    if (!rl.ok) {
      await logAuthAudit({ action: "signup_verify", email, ip, userAgent: ua, status: "rate_limited" });
    ...
    if (vErr || !data.session) {
      await logAuthAudit({ action: "signup_verify", email, ip, userAgent: ua, status: "error" });
    ...
    await logAuthAudit({ action: "signup_verify", email, ip, userAgent: ua, status: "ok" });
    return jsonRes;
  } catch {
    await logAuthAudit({ action: "signup_verify", email, ip, userAgent: ua, status: "error" });
