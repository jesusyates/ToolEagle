...
export const metadata: Metadata = {
  title: "Community Prompts | ToolEagle",
  description: "AI prompts from creators for TikTok, YouTube, and Instagram.",
  alternates: { canonical: `${BASE_URL}/community/prompts` }
};
      ...
      <div className="flex-1">
        <section className="container py-12">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-4 text-2xl font-semibold text-slate-900">Prompts</h1>
            <p className="mt-2 text-slate-600">Creator prompts for AI tools.</p>
            {posts.length === 0 ? (
              <p className="mt-8 text-slate-500">No prompts yet.</p>
            ) : (
              <ul className="mt-6 space-y-3">
                {posts.map((p) => {
                  ...
                  return (
                    <li key={p.id}>
                      <Link href={`/community/prompts/${p.slug}`} className="block rounded-xl border border-slate-200 px-4 py-3 hover:border-sky-300 transition">
