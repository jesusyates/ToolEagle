...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const data = await loadProgrammaticPage("ai_generator", slug, "en");
...
export default async function ProgrammaticAiGeneratorPage({ params }: Props) {
  const { slug } = await params;
  const data = await loadProgrammaticPage("ai_generator", slug, "en");
  if (!data) notFound();
  ...
  return (
    <ProgrammaticPageTemplate
      ...
      slug={data.slug}
      locale="en"
      sourcePagePath={pathForProgrammaticPage("ai_generator", slug, "en")}
