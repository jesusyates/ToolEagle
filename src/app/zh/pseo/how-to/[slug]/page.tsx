...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const data = await loadProgrammaticPage("how_to", slug, "zh");
...
export default async function ZhProgrammaticHowToPage({ params }: Props) {
  const { slug } = await params;
  const data = await loadProgrammaticPage("how_to", slug, "zh");
  if (!data) notFound();
  ...
  return (
    <ProgrammaticPageTemplate
      ...
      slug={data.slug}
      locale="zh"
      sourcePagePath={pathForProgrammaticPage("how_to", slug, "zh")}
