...
export function LoginForm({
  ...
  async function handleSendLink(e: React.FormEvent) {
    ...
    setLoading(false);
    if (err) {
      const isRateLimit = /rate limit|too many|rate_limit/i.test(err.message);
      setRateLimited(isRateLimit);
      setError(isRateLimit ? t("rateLimitMessage") : err.message);
    } else {
      setSent(true);
      setRateLimited(false);
  ...
  return (
    <div className="flex-1 flex items-center justify-center py-12">
      <div className="w-full max-w-sm px-4">
        <h1 className="text-2xl font-semibold text-slate-900 text-center">{t("title")}</h1>
        <p className="mt-2 text-sm text-slate-600 text-center">{t("subtitle")}</p>

        {error && (
          <div
            className={`mt-4 p-4 rounded-xl ${rateLimited ? "bg-amber-50 border border-amber-200" : "bg-red-50"} text-sm`}
          >
          ...
          <div className="mt-8 p-5 rounded-2xl border border-sky-200 bg-sky-50/80 text-center">
            {status === "success" ? (
              ...
              <>
                ...
                <p className="mt-1 text-sm text-slate-600">{t("sentTo", { email })}</p>
        ...
        <p className="mt-6 text-center text-xs text-slate-500">
          <span>{t("termsBefore")}</span>{" "}
          <Link href={termsHref} className="text-sky-600 hover:underline">
            {t("termsLink")}
          </Link>
          <span>.</span>
        </p>
