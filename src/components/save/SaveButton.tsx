...
export function SaveButton({ exampleSlug, content, variant = "button" }: Props) {
  const [saved, setSaved] = useState(false);
  const [loading, setLoading] = useState(false);
  const [checked, setChecked] = useState(false);

  useEffect(() => {
    fetch(`/api/saves/check?exampleSlug=${encodeURIComponent(exampleSlug)}`)
      .then((r) => r.json())
      .then((d) => {
  ...
  async function handleClick() {
    ...
    setLoading(true);
    if (saved) {
      const res = await fetch(`/api/saves?exampleSlug=${encodeURIComponent(exampleSlug)}`, {
        method: "DELETE"
      });
