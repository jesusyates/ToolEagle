...
type ToolResultListCardProps = {
  ...
  onCopyAll: () => void;
  /** V72: Called when copy happens via getTextToCopy (for analytics, since onClick isn't called) */
  onCopyTrack?: (index: number) => void;
  /** V171.1 — After clipboard write succeeds (per-item or copy-all). */
  onAfterSuccessfulCopy?: () => void;
  onRegenerate?: () => void;
              ...
              <ToolCopyButton
                ...
                getTextToCopy={(btn) => {
                  ...
                  const raw = Array.from(sources)
                    ...
                    .filter(Boolean)
                    .join("\n\n---\n\n");
                  return raw ? textWithCopyBranding(raw) : null;
                }}
                onCopied={() => {
                  onCopyTrack?.(-1);
                  onAfterSuccessfulCopy?.();
                  ...
                  <div className="flex-1 min-w-0">
                    {isEditing ? (
                      ...
                      <div>
                        ...
                        <p className="mt-2 text-xs text-slate-500">{t("resultAttribution")}</p>
                      </div>
                    )}
                  </div>
                    ...
                    {isEditing ? (
                      <>
                        ...
                        <ToolCopyButton
                          ...
                          variant="primary"
                          getTextToCopy={() => (editText ? textWithCopyBranding(editText) : null)}
                          onCopied={() => {
                            onCopyTrack?.(index);
                            onAfterSuccessfulCopy?.();
                          }}
                          ...
                          getTextToCopy={(btn) => {
                            ...
                            const content =
                              el && "value" in el
                                ...
                                : el?.innerText?.trim() ?? "";
                            return content ? textWithCopyBranding(content) : null;
                          }}
                          onCopied={() => {
                            onCopyTrack?.(index);
                            onAfterSuccessfulCopy?.();
