...
export function ZhToolRecommendationBlock({ tools, keyword, ctaIndex = 0 }: Props) {
  ...
  const handleClick = (tool: AffiliateTool) => {
    fetch("/api/zh/analytics", {
      ...
      body: JSON.stringify({
        ...
        event_data: { tool_id: tool.id, keyword: keyword || null }
      })
    }).catch(() => {});
          ...
          return (
            <div
              ...
              className={`rounded-xl border-2 bg-white p-4 shadow-sm hover:shadow-md transition ${
                tool.isBestChoice ? "border-amber-400 ring-2 ring-amber-200" : "border-amber-200"
              }`}
            >
