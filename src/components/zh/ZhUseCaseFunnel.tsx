...
import { getToolsForUseCaseFunnel } from "@/config/affiliate-tools";
...
function trackToolClick(tool: AffiliateTool, keyword?: string, pageSlug?: string) {
  fetch("/api/zh/analytics", {
    ...
    body: JSON.stringify({
      ...
      event_data: { tool_id: tool.id, keyword: keyword || null, page_slug: pageSlug || null, source: "use_case_funnel" }
    })
  }).catch(() => {});
}
...
export function ZhUseCaseFunnel({ tools, keyword, pageSlug, hasAffiliate }: Props) {
  ...
  if (!funnelTools.length || !hasAffiliate) return null;
          ...
          <li key={s.tool.id} className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <button
                type="button"
                onClick={() => {
                  trackToolClick(s.tool, keyword, pageSlug);
                  if (s.tool.url) window.open(s.tool.url, "_blank", "noopener,noreferrer");
                }}
